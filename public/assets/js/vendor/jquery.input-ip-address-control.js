        ...
        $.fn.extend({
                ...
                ipAddress: function(s) {
                        ...
                        return $(this).each(function(){
                                ...
                                ctx.input = $(this);
                                var model = ctx.input.attr('ng-model') !== undefined ? ctx.input.attr('ng-model').toString().split('.') : null;
                                if( getScopeValue() =='' || !isIp(ctx.input.val())){
                                    
                                        //ng.element(ctx.input[0]).val( s.label ).triggerHandler('input');
                                    setScopeValue(s.label,true);
                                }
                                ctx.input.attr('maxlength',(s.v==4? 15 : 39)).attr('size',(s.v==4? 15 : 39));
                                
                                function setScopeValue(val,init){
                                    if(s.scope === undefined || model===null) return;
                                   
                                    if(model.length===1){
                                        s.scope[model[0]] = val;
                                    }
                                    if(model.length===2){
                                        s.scope[model[0]][model[1]] = val;
                                    }
                                    if(model.length===3){
                                       s.scope[model[0]][model[1]][model[2]] = val; 
                                    }
                                }; 
                                function getScopeValue(){
                                    ...
                                    if(model.length===1){
                                        return s.scope[model[0]];
                                    }
                                    if(model.length===2){
                                        return s.scope[model[0]][model[1]];
                                    }
                                    if(model.length===3){
                                       return s.scope[model[0]][model[1]][model[2]]; 
                                    }
                                ...
                                function loadCtx() {
                                        ...
                                        ctx.ip = isIp(unmask(getScopeValue())) ? unmask(getScopeValue()) : ctx.ip ;
                                        ...
                                        switch(e.keyCode) {
                                                case 8: //backspace
                                                        if(ctx.buffer[ctx.cursor.end-1] != s.separator) {
                                                                ...
                                                                setScopeValue(ctx.buffer.join(""));
                                                                //s.scope[ctx.input.attr('ng-model')] = ctx.buffer.join("");
                                                        }
                                ...
                                ctx.input.bind('keydown',function(e){
                                        ...
                                        if( isIp(ip) ){
                                            ctx.input.val(ip);
                                            setScopeValue(ip);
                                            //s.scope[ctx.input.attr('ng-model')] = ip;
                                        }
                                        else {
                                        ...
                                        setTimeout(function() {
                                                ...
                                                if(ctx.input.val() != s.label){
                                                    ...
                                                    setScopeValue(ctx.ip);
                                                    //s.scope[ctx.input.attr('ng-model')] = mask(ctx.ip);
                                                }
                                                if(ctx.input.val() ===''){
                                                    setScopeValue(s.label);
                                                }
                                                ctx.input.caret(0);
                                        }, 0);
