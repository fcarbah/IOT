(function () {
    ...
    .module('ctrl.alarm', ['cfp.loadingBar'])
    
    .controller('MessageCtrl', ['$rootScope','$scope','cfpLoadingBar', '$timeout', '$state','HttpService', function ($rootScope,$scope,cfpLoadingBar,$timeout,$state,HttpService) {
            
        $scope.alarmMessages =[];

        $scope.loadMessages = function(){
            HttpService.get('/alarm/messages',$scope.setMessages);
        };
        $scope.setMessages = function(res){
            if(!res.error){
                $scope.alarmMessages = res.data;
            }
        };
        ...
        $rootScope.$on('$stateChangeSuccess', function (event, data) {
            ...
            if( $state.is('app.admin.messages.message') ) {
                    $rootScope.expanded = true;
            }
        });
    ...
    .controller('SingleMessageCtrl', ['$scope', '$state', '$rootScope', function ($scope, $state, $rootScope) {
        $rootScope.expanded = false;

        if ( $state.params.id ) {
            $scope.$parent.alarmMessages.forEach(function (mail) {
        ...
        $scope.cancel = function (e) {
            e.preventDefault();
            $rootScope.expanded = true;
            $state.go('app.admin.alarm.messages');
        };
    }]);
